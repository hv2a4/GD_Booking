...
const AmenitiesTypeRoom = () => {
  ...
  const getDataAmenitiesTypeRoom = async () => {
    try {
      const response = await axios.get(
        'http://localhost:8080/api/amenities-type-room/getAll',
      )
  ...
  useEffect(() => {
    if (isRefreshTable) {
      getDataAmenitiesTypeRoom() // Gọi API để lấy dữ liệu khi isRefreshTable là true
              ...
              <input
                ...
                onChange={() => {
                  const allSelected =
                    ...
                    dataAmenitiesTypeRoom.length
                  setSelectedAmenitiesTypeRoom(
                    allSelected
                      ? []
                      : dataAmenitiesTypeRoom.map(
                          ...
                          <div className="container-fluid">
                            ...
                            >
                              <Row className="mb-4 align-items-start">
                                ...
                                <Col md={4}>
                                  <p>
                                    <strong>Mã tiện nghi loại phòng: </strong>
                                    {id}
                                  </p>
                                  <p>
                                    <strong>Tên tiện nghi loại phòng: </strong>{' '}
                                    {amenitiesTypeRoomName}
                                  </p>
                                  <p>
                                    <strong>Icon:</strong>{' '}
                              ...
                              <div className="d-flex justify-content-end">
                                <AmenitiesTypeRoomFormModal
                                  ...
                                  refreshTable={handleTakeCheckRefeshTable}
                                 
                                />
                                <DeleteAmenitiesTypeRoomModal
                                  id={id}
                                  amenitiesTypeRoomName={amenitiesTypeRoomName}
                                  refreshTable={handleTakeCheckRefeshTable} // Truyền hàm này đúng cách
                                />
                              </div>
                            </div>
                          </div>
