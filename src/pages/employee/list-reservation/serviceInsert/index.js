...
import { addBookingRoomServiceRoom, deleteService } from "../../../../services/employee/booking-room";
import AlertComfirm from "../../../../config/alert/comfirm";
import { formatCurrency } from "../../../../config/formatPrice";

const ProductServiceModal = ({ handleClose }) => {
        ...
        setSelectedProducts((prev) =>
            prev.map((p) =>
                p.id === id
                    ? {
                        ...p,
                        quantity: Math.max(1, p.quantity + (increment ? 1 : -1)),
                    }
                    : p
            )
        );
